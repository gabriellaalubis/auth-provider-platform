...
<section class="card"><div class="card-header"><div><p class="eyebrow">Provider notifications</p><h2>Processed events</h2></div></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Event ID</th><th>Type</th><th>Processed at</th><th>Result</th></tr></thead><tbody>${events.length ? events.map((event) => `<tr><td class="mono" title="${this.escape(event.eventId)}">${this.short(event.eventId)}</td><td>${this.escape(event.eventType)}</td><td class="mono small">${this.date(event.processedAt)}</td><td>${this.escape(event.result)}</td></tr>`).join('') : '<tr><td class="empty-state" colspan="4">No provider events processed yet.</td></tr>'}</tbody></table></div></section></main>
